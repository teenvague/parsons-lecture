...
After B/C pairing, unused B images may form additional pairs with exactly one portrait and one landscape; square images do not qualify. B/B pairing never repeats an image. These rows occupy the same pair slots in the A-separated rhythm. They alternate bottom and top image-edge alignment, use zero horizontal gap, and preserve natural proportions. Column proportions are chosen to favor unequal image heights, following Loosies. The entire row scales together to keep edges touching, including on mobile; captions do not change the shared image baseline. A B/B row appears only when compatible unused B images are available. The existing temporary B-reuse setting for B/C pairs is unchanged.
